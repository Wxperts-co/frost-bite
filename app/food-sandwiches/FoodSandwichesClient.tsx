...
import { FaHamburger, FaCheese, FaUtensils, FaDrumstickBite, FaBacon } from 'react-icons/fa';
...
export default function FoodSandwichesClient() {
  const [activeTab, setActiveTab] = useState("burgers");
  
  const currentCategory = foodCategories.find(c => c.id === activeTab);
  const currentMenu = menuData[activeTab as keyof typeof menuData] || [];
  const Icon = currentCategory?.icon || FaHamburger;
  ...
  return (
    <div className="relative z-10 container mx-auto px-4">
      ...
      {/* Category Tabs */}
      <div className="relative z-20 pb-4 bg-transparent mb-12">
          ...
          {foodCategories.map((cat) => {
            ...
            return (
              <button
                ...
                className={`flex items-center gap-2 px-5 py-2.5 rounded-full font-semibold transition-all duration-300 shadow-sm hover:scale-[1.02] ${
                  isActive 
                    ? 'bg-gradient-to-r from-[#c07f07] to-[#046069] text-white shadow-lg shadow-[#c07f07]/20' 
                    : 'bg-gray-100 text-gray-700 hover:bg-[#c07f07]/10 hover:text-[#c07f07]'
                }`}
              >
                <CatIcon size={16} />
        ...
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
          ...
          <div className="text-center mb-8">
            <h2 className="text-2xl md:text-3xl font-bold text-[#1e1e1e] mb-2 flex justify-center items-center gap-2">
              <Icon className="text-[#c07f07]" /> {currentCategory?.name}
            </h2>
